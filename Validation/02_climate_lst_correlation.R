...
# Monthly climate table: an Excel workbook or a CSV. The first two columns are
# assumed to be year and month; the temperature columns are matched by name.
#
# ### EDIT ### Leave as NULL to search automatically for a spreadsheet or CSV
# whose name suggests climate data, beside these scripts and one level up.
PATH_CLIMATE <- NULL
# e.g. PATH_CLIMATE <- "~/my_project/climate/monthly_climate.xlsx"

# ### EDIT ### Column names inside that file, copied EXACTLY as they appear,
# including accents, spaces and parentheses.
#
# Set any of them to NULL to have the script look for a column whose name
# contains the obvious keywords ("mean"/"med" + "temp", and so on). Automatic
# matching is a convenience for a first run; for anything you intend to publish,
# write the names out explicitly so the choice is on the record.
#
# If a name does not match, the script stops and prints every column it did
# find, so the fix is to copy the right one from that list.
COL_TEMP_MEAN <- NULL
COL_TEMP_MIN  <- NULL
COL_TEMP_MAX  <- NULL
# -----------------------------------------------------------------------------
...
# 2. LOAD CLIMATE DATA
# -----------------------------------------------------------------------------
# --- locate the climate file -------------------------------------------------
if (is.null(PATH_CLIMATE) || !nzchar(PATH_CLIMATE)) {
  # Automatic search: a spreadsheet or CSV whose name suggests climate data,
  # beside these scripts, one level up, or in an obvious sub-folder.
  roots <- unique(c(.suite_dir, dirname(.suite_dir),
                    file.path(dirname(.suite_dir), c("climate", "clima",
                                                     "Clima_mensal", "data"))))
  roots <- roots[dir.exists(roots)]
  hits  <- unlist(lapply(roots, function(r)
    list.files(r, pattern = "\\.(xlsx|xls|csv)$", full.names = TRUE,
               ignore.case = TRUE)))
  hits <- hits[grepl("clim|weather|temp|meteo", basename(hits),
                     ignore.case = TRUE)]
  if (length(hits) == 0)
    stop("No climate file given and none found automatically.\n",
         "  Searched in:\n    ", paste(roots, collapse = "\n    "),
         "\n  Set PATH_CLIMATE at the top of this script.")
  PATH_CLIMATE <- hits[1]
  message(sprintf("%s Climate file found automatically: %s",
                  TAG, PATH_CLIMATE))
}
PATH_CLIMATE <- path.expand(PATH_CLIMATE)
if (!file.exists(PATH_CLIMATE))
  stop("Climate file not found:\n  ", PATH_CLIMATE,
       "\nEdit PATH_CLIMATE at the top of this script.")

# --- read it, whatever format it is ------------------------------------------
climate_raw <-
  if (grepl("\\.csv$", PATH_CLIMATE, ignore.case = TRUE)) {
    read_any_csv(PATH_CLIMATE)
  } else {
    if (!requireNamespace("readxl", quietly = TRUE))
      stop("Reading an Excel climate file requires the readxl package.\n",
           "  Either install it, or export the sheet to CSV and point ",
           "PATH_CLIMATE at that.")
    readxl::read_excel(PATH_CLIMATE)
  }
...
# --- resolve the temperature column names ------------------------------------
# When a COL_TEMP_* is NULL, look for a column whose name contains the expected
# keywords. The choice is always printed, because a silently wrong column would
# yield a plausible-looking but meaningless correlation.
.pick_col <- function(explicit, keywords, label) {
  if (!is.null(explicit) && nzchar(explicit)) return(explicit)
  nm  <- names(climate_raw)
  hit <- nm[grepl("temp", nm, ignore.case = TRUE) &
            grepl(keywords, nm, ignore.case = TRUE)]
  hit <- setdiff(hit, c("year", "month"))
  if (length(hit) == 0)
    stop("Could not identify the ", label, " temperature column automatically.",
         "\n\nColumns present:\n  ", paste(nm, collapse = "\n  "),
         "\n\nSet COL_TEMP_* explicitly at the top of this script.")
  if (length(hit) > 1)
    message(sprintf("%s   several candidates for %s temperature (%s); using '%s'",
                    TAG, label, paste(hit, collapse = ", "), hit[1]))
  hit[1]
}
COL_TEMP_MEAN <- .pick_col(COL_TEMP_MEAN, "mean|med|avg|m\u00e9d", "mean")
COL_TEMP_MIN  <- .pick_col(COL_TEMP_MIN,  "min",                 "minimum")
COL_TEMP_MAX  <- .pick_col(COL_TEMP_MAX,  "max",                 "maximum")
message(sprintf("%s   using columns: mean='%s' | min='%s' | max='%s'",
                TAG, COL_TEMP_MEAN, COL_TEMP_MIN, COL_TEMP_MAX))
